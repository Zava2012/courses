...
5. Создать пользователя в AWS с `Programmatic access` для генерации ключей, а также добавить этому пользователю следующие политики:
    ...
    AmazonVPCFullAccess
    ```
6. Подготовить через terraform минимально жизнеспособную инфраструктуру с минимальными денежными затратами, изменив в файле `cluster.tfvars` следующие строки:
    ```
    #Global Vars
    ...
    ```
7. Развернуть настроенную инфраструктуру, используя следующую команду:
    - `terraform apply -var-file=cluster.tfvars ../../contrib/terraform/aws/`
8. После выполнения этой команды и предпроверки плана terraform необходимо будет ввести слово `yes`. После успешного создания инфраструктуры в папке, где была запущена команда, будет создан файл `hosts`, в котором описаны данные для того, чтобы настраивать на базе созданной инфраструктуры кластер Kubernetes.
